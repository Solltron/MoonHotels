...
{
    private static readonly IDictionary<ErrorMessageCode, string> ErrorMessageDescriptions =
        InitErrorMessages();
    ...
    public ErrorMessage(ErrorMessageCode code, ExternalError externalError = default)
    {
        ...
        ErrorMessageDescriptions.TryGetValue(code, out var errorDescription);

        Description = errorDescription ?? throw new Exception($"Error description not found for code {code}");
    }
